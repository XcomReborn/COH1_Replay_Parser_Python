...
class COH_Replay_Parser:
    """Parses a company of heroes 1 replay to extract as much useful
        ...
        try:
            if self.data:
                ...
                theInt = int.from_bytes(
                    fourBytes, byteorder='little', signed=False)
                return theInt
    ...
    def read_LengthString(self):
        """Reads the first 4 bytes containing the string length and then the
         rest of the string."""
        try:
            if self.data:
                stringLength = self.read_UnsignedLong4Bytes()
                theString = self.read_2ByteString(
    ...
    def read_ASCIIString(self, stringLength=0) -> str:
        """Reads a byte array of spcified length and attempts to convert it
         into a string."""
        try:
            if self.data:
                theBytes = bytearray(
    ...
    def read_NULLTerminated_2ByteString(self) -> str:
        """Reads a Utf-16 little endian character string until the first two
         byte NULL value."""
        try:
            if self.data:
                characters = ""
    ...
    def processData(self):
        ...
        self.read_ASCIIString(stringLength=8)  # cohrec
        ...
        self.seek(-28, 1)
        # sets file pointer back to start of relic chunky
        self.seek(self.chunkyHeaderLength, 1)
        # seeks to begining of FOLDPOST
        ...
        self.read_UnsignedLong4Bytes()  # unknown
        self.read_UnsignedLong4Bytes()  # 3 chunkyVersion
        self.read_UnsignedLong4Bytes()  # unknown
    ...
    """
    Processes the date string that is in a different format depending on the
     game locale (US, EURO and ASIAN dates strings follow a different
      string format)
    """
